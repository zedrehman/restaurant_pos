...
$RoleSubMenuList = session('RoleSubMenuList');
//echo '<pre>';
//print_r($RoleMenuList);
//echo '</pre>';
?>
...
<ul class="metismenu" id="menu">
    @foreach ($RoleMenuList as $MItems)

    @if($MItems->is_has_sub_menu==0)
    <li id="{{ $MItems->id_name }}" data-id="{{ $MItems->id }}">
        <a href="{{url('')}}{{ $MItems->menu_url }}" aria-expanded="false">
            <i class="{{ $MItems->icon }}"></i>
            <span class="nav-text">{{ $MItems->menu_name }}</span>
        </a>
    </li>
    @else
    <?php
    $SubMenusItems = array_filter($RoleSubMenuList, function ($sitems) use ($MItems) {
